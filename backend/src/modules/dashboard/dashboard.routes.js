...
router.get("/kpis", requirePermission(PERMISSIONS.DASHBOARD_READ), async (req, res, next) => {
  try {
    ...
    const [todaysSales, monthlySales, pendingPurchaseOrders, payments, lowStockAlerts] = await Promise.all([
    ...
    const payload = {
      ...
      lowStockItemCount: lowStockAlerts?.[0]?.count || 0,
      pendingPurchaseOrders,
      gstPayablePaise: 0, // computed in gstr3b report
    };
...
router.get("/sales-chart", requirePermission(PERMISSIONS.DASHBOARD_READ), async (req, res, next) => {
  try {
    const prisma = getPrisma();
    ...
    for (const r of records) {
      ...
      if (byDate.has(key)) byDate.get(key).sales += r.totalAmount / 100;
    }
...
router.get("/top-items", requirePermission(PERMISSIONS.DASHBOARD_READ), async (req, res, next) => {
  try {
    const prisma = getPrisma();
